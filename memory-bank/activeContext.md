...
Snyk lab gate is green via `snyk auth` (`justusaleksu`). Do not put a UUID SNYK_TOKEN in `.env` — it overrode OAuth with 401. `./scripts/scan-cve.sh` prefers CLI login.
