...
router.post('/submissions', async (req, res) => {
    try {
        const { name, address, operator_name, latitude, longitude, connector_note, submitted_by } = req.body ?? {};
 ...
 * App-owner inbox.
 */
router.get('/submissions', requireOwner, async (req, res) => {
    try {
        ...
        const rows = await prisma.stationSubmission.findMany({
            ...(status === 'ALL'
                ...
                : {
                    where: {
                        status: Object.values(SubmissionStatus).includes(status)
                            ? status
 ...
 */
router.post('/submissions/:id/confirm', requireOwner, async (req, res) => {
    try {
...
router.post('/submissions/:id/reject', requireOwner, async (req, res) => {
    try {
        const id = req.params.id;
        if (!id) {
            return res.status(400).json({ error: 'id is required' });
...
router.post('/check-in', async (req, res) => {
    try {
        const { station_id, reporter_id, working, free_connectors, latitude, longitude } = req.body ?? {};
...
//# sourceMappingURL=crowdRoutes.js.map