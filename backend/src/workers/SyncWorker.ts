...
export class SyncWorker {
    ...
    private viaLietuva: ViaLietuvaService;
    private dailyTask: cron.ScheduledTask | null = null;
    private viaTask: cron.ScheduledTask | null = null;
    ...
    start(): void {
        console.log(
            '[SyncWorker] Starting… Via Lietuva OCPI every 5 min; Open Charge Map daily 02:00',
        );
    ...
    private async syncViaLietuva(): Promise<boolean> {
        try {
    ...
    private async syncOcm(options?: { skipLithuania?: boolean }): Promise<void> {
        try {
            const exclude = options?.skipLithuania ? ['LT'] : [];
            console.log(
                `[SyncWorker] Fetching Open Charge Map…` +
            ...
            if (options?.skipLithuania) {
                ...
                if (removedLt.count) {
                    console.log(
                        `[SyncWorker] Removed ${removedLt.count} OCM Lithuania rows (Via Lietuva is source)`,
                    );
                }
            }
    ...
    private async upsertCatalogue(stations: StationData[]): Promise<void> {
        const now = new Date();
        let i = 0;
        for (const incoming of stations) {
            ...
            if (incoming.connectors.length === 0) continue;
