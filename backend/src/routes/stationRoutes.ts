...
function catalogueSource(externalId: string | null): string {
    if (externalId?.startsWith('vl:')) return 'via_lietuva';
    if (externalId?.startsWith('ocm:')) return 'ocm';
    if (externalId?.startsWith('user:')) return 'user';
    return 'unknown';
}

function displayTariff(connectors: { tariff: string | null }[]): string | null {
...
function page(escapedTitle: string, body: string): string {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8"/>
